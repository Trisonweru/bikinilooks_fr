...
const ProductDetails: React.FC<ProductDetailsProps> = ({ price, reviews, relatedProducts }: any) => {
  ...
  const { addItemToCart, decrementItemQuantity, incrementItemQuantity, items } = useCart();
      ...
      <div className="flex mb-8">
        ...
        <div className="w-1/2 pl-8">
          ...
          <div className="flex items-center mb-4">
            <button onClick={() => decrementItemQuantity(data?.ID.toString())} className="px-2 py-1 bg-gray-200 rounded">-</button>
            <input
              type="text"
              value={items?.length}
              readOnly
            ...
            />
            <button onClick={() => incrementItemQuantity(data?.ID.toString())} className="px-2 py-1 bg-gray-200 rounded">+</button>
            <button
