...
const ProductDetails: React.FC<ProductDetailsProps> = ({ reviews, relatedProducts }: any) => {
  ...
  const getProduct = async () => {
    ...
    if (res?.data?.status == 200) {
     ...
     const getProduct = async () => {
    const res = await axios.post('/api/category', { category:1 });
  ...
  return (
    <>
      {data?<div className="container mx-auto p-4 pt-28">
    ...
    </div>:<div className='container bg-white min-h-screen w-full flex justify-center items-center ' ><ImSpinner2 className='animate-spin' /></div>}
    </>
  );
};
