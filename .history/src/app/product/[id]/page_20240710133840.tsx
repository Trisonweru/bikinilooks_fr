...
const ProductDetails: React.FC<ProductDetailsProps> = ({ price, reviews, relatedProducts }: any) => {
  ...
  const handleAddToCart = () => {
    ...
    addItemToCart({ id: data?.ID.toString(), title: data?.product_name, price: data?.price, quantity, totalPrice: 0, image: data?.product_image });
  };

  return (
    <div className="container mx-auto p-4 pt-28">
      <div className="flex mb-8">
        <img src={data?.product_image} alt={data?.product_name} className="w-1/2 object-cover rounded" />
        <div className="w-1/2 pl-8">
          ...
          <div className="flex items-center mb-4">
            <button onClick={() => decrementItemQuantity(data?.ID)(-1)} className="px-2 py-1 bg-gray-200 rounded">-</button>
            <input
              type="text"
              value={quantity}
              readOnly
            ...
            />
            <button onClick={() => decrementItemQuantity(data?.ID)} className="px-2 py-1 bg-gray-200 rounded">+</button>
            <button
