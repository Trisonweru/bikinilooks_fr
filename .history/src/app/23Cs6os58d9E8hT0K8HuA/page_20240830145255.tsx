...
import { NEXTAUTH_URL } from '@/lib/utils';
...
export default function Form() {
  ...
  const getProducts = async () => {
    setLoading(true)
    const res2 = await fetch(`${NEXTAUTH_URL}/api/hello`);
    const res = await res2.json()
  ...
  const getOrders = async () => {
    setLoading(true)
    const res2 = await fetch(`${NEXTAUTH_URL}/api/orders`);
    const res = await res2.json()
              ...
              <Accordion key={index}>
                ...
                >
                  <div className='space-x-3 font-semibold'>
                    ...
                    <span className='text-xs text-gray-500'>{new Date(item?.UpdatedAt).toLocaleString()}</span>
                  </div>
