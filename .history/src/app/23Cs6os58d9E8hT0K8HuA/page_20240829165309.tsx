              ...
              <Accordion key={index}>
                ...
                >
                  <div className='space-x-3'>
                    ...
                    <span className={item?.status === "COMPLETE" ? 'font-semibold text-green-500' : item?.status === "PENDING" ? "font-semibold " : 'font-semibold'}>{item?.status}</span>
                    <span className='text-sm text-g'>{new Date(item?.CreatedAt).toLocaleString()}</span>
                  </div>
