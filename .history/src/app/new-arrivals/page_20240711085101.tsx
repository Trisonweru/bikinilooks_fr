...
function NewArrivals() {
  ...
  const getProduct = async () => {
    const res2 = await fetch(`/api/product/4`);
  ...
  return (
    <div className="container mx-auto px-4 pt-14 bg-white">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {data?.map((card: { ID: React.Key | null | undefined; product_image: string; product_category: string; product_name: string; price: number; }) => (
          <ServiceCard
            key={card.ID}
