...
const Navbar: React.FC = () => {
  ...
  const handleClick:an = (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handlePopOver = (id: any) => {
    setIsPopoverVisible(true);
  }

  const handlePopOverOut = (id: any) => {
    ...
    <nav className={`fixed w-4/5 top-0 z-50 transition-all duration-300 ${isScrolled ? 'h-16' : 'h-24'} bg-white shadow-lg`}>
      <div className="h-full flex items-center justify-between px-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative" onMouseEnter={handlePopOver} onMouseLeave={handlePopOverOut} >
            <Link href={"/cart"}>
