...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed w-full top-0 z-50 transition-all duration-300 ${isScrolled ? 'h-20' : 'h-36'} bg-white shadow-lg`}>
      <div className="h-full flex items-center justify-between px-4 lg:px-8">
        ...
        {mobileMenuOpen&&<div className="flex items-center">
          <input
            type="text"
            placeholder="Search..."
        ...
        <div className="flex items-center justify-center flex-grow lg:flex-grow-0">
          <Link href={"/"}>
            <Image src='/images/logo.png' layout="fixed" alt='logo' height={40} width={100} className={`font-bold transition-transform duration-300 ${isScrolled ? 'scale-75' : 'scale-100'}`} />
          </Link>
        </div>
      ...
      <div className="bg-[#752A78] h-12 flex items-center justify-center text-white text-sm shadow-left-right lg:flex hidden">
        <div className="flex space-x-8 items-center">
          ...
          >
            ...
            >
              {data?.map((item: any) => (
                <Link key={item.ID} href={`/products/${item?.name?.split(" ").join("_")}/${item?.ID}`} >
                  <MenuItem onClick={() => {
