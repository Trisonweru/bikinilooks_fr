...
const Navbar: React.FC = () => {
  ...
  const subtotal: number = items.reduce((total, item) => total + item.totalPrice * item.quantity, 0);
              ...
              <Box sx={{ border: 1, p: 1, bgcolor: 'background.paper', width: '300px' }}>
                {items.length < 1 ? (
                  ...
                  <div>
                    ...
                    <hr />
                    <div className="mt-4">
                      <p className="text-lg font-bold">Subtotal:{formatCurrency(subtotal.toFixed(2).toString())}</p>
                      <hr />
