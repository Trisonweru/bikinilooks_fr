      ...
      <div className="bg-[#752A78] h-12 flex items-center justify-center text-white text-sm shadow-left-right">
        <div className="flex space-x-8 items-center">
          ...
          >
            ...
            >
              {data?.map((item: any) => {
                return <Link href={`/${item?.name?.split(" ").join("_")}/${item?.ID}`} onClick={()=>                addPathname(`/${item?.name?.split(" ").join("_")}/${item?.ID}`)}><MenuItem onClick={handleClose}>{item?.name}</MenuItem></Link>
              }
              )}
