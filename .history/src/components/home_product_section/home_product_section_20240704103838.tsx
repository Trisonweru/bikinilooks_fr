...
function ProductSection({title, slides}:any) {
  ...
  return (
    <div className="bg-[#fff] py-0 flex flex-col space-y-2 items-center">
      <div>
        <h2 className="text-2xl font-semibold text-[#BB4C48] text-center">
          {title}
        </h2>
      </div>
      <div className="w-full h-40 md:w-[80%] flex md:flex-row mb-10 flex-col space-y-4 md:space-y-0 md:space-x-4">
        <Carousel
          opts={{
            align: "center",
            loop: true,
        ...
        >
          <CarouselContent className="-ml-1">
            {slides.map((card:any, index:number) => (
              ...
              >
                <ServiceCard title={""} image={card?.image} link />
              </CarouselItem>
            ))}
          </CarouselContent>
          <CarouselPrevious />
