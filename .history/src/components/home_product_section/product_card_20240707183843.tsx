...
const ServiceCard: React.FC<ServiceCardProps> = ({ id, image, category, title, price }) => {
  ...
  const addToCartHandler = () => {
    addItemToCart({ id, title, price, quantity: 1, totalPrice: 0 });
  };
...
className="mt-2 w-full border border-[#752A78] text-[#752A78] hover:bg-[#752A78] hover:text-white px-4 py-1 text-sm rounded">Add to Cart</button>
      </div>
