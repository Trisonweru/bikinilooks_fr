...
const Cart: React.FC = () => {
  ...
  const handleWhatsAppOrder = () => {
    const orderDetails = items
      .map(
        (item) =>
          `${item.image} \n\n${item.title} (x${item.quantity}) - Ksh${item.totalPrice}`
      )
      .join("\n");
    ...
    const whatsappLink = `https://wa.me/254768510515?text=${encodeURIComponent(
      message
    )}`;
  ...
  const tkn = () => {
    const tkon: any = window.localStorage.getItem("token");
    setToken(tkon);
  };

  return (
    <div className="flex p-4 space-x-4 pt-28 bg-white min-h-screen text-black">
