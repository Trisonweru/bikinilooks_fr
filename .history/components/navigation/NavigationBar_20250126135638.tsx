...
import { useAppCtx } from "@/pages/context/AppContext";
...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="h-full flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="relative">
            <div className="hidden md:flex items-center">
              <input
                ...
                placeholder="Search..."
                className="border px-4 py-2 border-slate-600 text-black rounded-full text-sm w-full focus:border-[#752A78] focus:outline-none focus:ring-1 focus:ring-slate-600"
              />
            </div>
            {/* Results dropdown */}
            {searchTerm && (
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden md:flex">
            {!tkn ? (
              <Link href={"/auth/login"}>
                <button className="border-r pr-4 border-gray-300 text-gray-600 text-sm">
                  Login
                </button>
              </Link>
            ) : (
          ...
          >
            <Link href={"/cart"}>
              <button
                className="flex items-center space-x-2 text-gray-600 text-sm hover:text-gray-800"
                aria-describedby={id}
              >
                <FaShoppingCart />
                <span>Cart / {formatCurrency(subtotal)}</span>
                <span className="bg-[#752A78] text-white rounded-full h-5 w-5 flex items-center justify-center">
                  {items.length}
                </span>
              </button>
            </Link>
              ...
              >
                {items?.length < 1 ? (
                  <div>
                    ...
                    <button className="mt-4 text-white bg-red-500 px-4 py-1 rounded-md ">
                      Close
                    </button>
                  </div>
                ) : (
                  <div className="text-black">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex justify-between w-full">
                        <button className="mt-2 text-white bg-red-500 px-4 py-1 rounded-md ">
                          Close
                        </button>
                        <Link href="/cart">
      ...
      <div className="bg-[#752A78] h-12 flex items-center justify-center text-white text-sm shadow-left-right lg:flex hidden">
        <div className="flex space-x-8 items-center">
          <Link
            href={"/"}
            className={
              pathname == "/"
                ? "border px-4 py-1 rounded-full font-normal"
            ...
            {data && (
              ...
              >
                {data?.map((item: any) => (
                  <Link
                    key={item.ID}
                    href={`/products/${item?.name?.split(" ").join("_")}/${
                      item?.ID
                    }`}
                  >
                    <MenuItem
                      onClick={() => {
                        addPathname(
                          `/${item?.name?.split(" ").join("_")}/${item?.ID}`
                        );
                        handleClose();
                      }}
                    >
      ...
      {mobileMenuOpen && (
        <div className="bg-gray-400 flex flex-col items-center lg:hidden text-white text-sm shadow-left-right p-4 space-y-4">
          <Link
            href={"/"}
            className={
              pathname == "/"
